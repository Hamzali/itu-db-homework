...
def init_course_controller(app, course_model, building_model, faculty_model, course_building_model):
		...
		if request.method == "POST":
			...
			for opt in course_codes:
				if len(opt.string) > 0:
					...
					soup = BeautifulSoup(content, "lxml")
					try:
						table = soup.body.table.contents[1].contents[0].contents[5].contents[1].contents[3].contents[13].find_all("tr")
					...
					for elem in table:
						if key >= 2:
							...
							try:
								course_model.create(data={"crn": int(crn), "code": code, "name": title})
							except:
								continue
							...
							for d in days:
								...
								if cday:
									...
									start_time = times[0]
									end_time = times[1]
									b_result = building_model.find(query=("code='%s'" % buildings[i]))
									if len(b_result) > 0:
	...
	def sync_buildings():
		if request.method == "POST":
			response = requests.get("http://www.sis.itu.edu.tr/tr/sistem/bina_kodlari.html")
			response.encoding = "windows-1254"
			content = response.text
			soup = BeautifulSoup(content, "lxml")
			...
			for tr in table:
				if tr.string != "\n":
					...
					if len(result) == 0:
						building_model.create(data={"name": name, "code": code})
		return "done"
			...
			for opt in faculty_options:
				if opt != "\n":
					...
					if len(code) > 0:
						...
						if len(result) == 0:
							faculty_model.create(data={"name": name, "code": code})
			return "done"
